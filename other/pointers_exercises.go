...
- iterate through arr
	...
	take arr on start and arr on end
	- check if they overlap
		- if yes:
		overlap = true
			end++
	- if not:
		if overlap = true
			- merge push the slice[start:end-1] to final
		- start = end
		- end = end+1
		- overlap = false
		- push arr to final
	...
	overlap:
	- the first element of second array is between two elemnt of the first arr
	(inclusive)
	[1,3]

	combine:
	- first el from teh first array
	- last el from teh last arry
